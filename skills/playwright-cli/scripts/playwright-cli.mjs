#!/usr/bin/env node
/**
 * Playwright CLI - 浏览器自动化命令行工具
 * 封装 Playwright 功能，支持 Chrome/Firefox/Safari，支持导航、截图、点击、填写表单等操作
 */

import { chromium, firefox, webkit, devices } from 'playwright';
import fs from 'fs';
import path from 'path';
import { fileURLToPath } from 'url';

// 获取 __dirname (ESM 中没有)
const __filename = fileURLToPath(import.meta.url);
const __dirname = path.dirname(__filename);

// 命令行参数解析
const args = process.argv.slice(2);
const command = args[0];

// 全局状态
let browser = null;
let page = null;
let context = null;
let currentBrowserType = 'chromium';

// 帮助信息
function showHelp() {
  console.log(`
Playwright CLI - 浏览器自动化工具
支持 Chrome (Chromium)、Firefox、Safari (WebKit)

用法: playwright-cli <command> [options]

命令:
  navigate <url>                    导航到指定 URL
  screenshot [name]                 截取屏幕截图
  click <selector>                  点击元素
  fill <selector> <value>           填写输入框
  select <selector> <value>         选择下拉菜单
  hover <selector>                  悬停元素
  evaluate <code>                   执行 JavaScript 代码
  wait <selector>                   等待元素
  text <selector>                   获取元素文本
  attr <selector> <name>            获取元素属性
  press <key>                       按下按键
  scroll-to <selector>              滚动到元素
  scroll-to-bottom                  滚动到底部
  close                             关闭浏览器

选项:
  --headless                        无头模式（默认）
  --headed                          有界面模式
  --browser <name>                  选择浏览器: chromium/firefox/webkit (默认: chromium)
  --viewport <w>x<h>                设置视口大小，如 1280x720
  --mobile <device>                 模拟移动设备，如 "iPhone 14"
  --fullpage                        截取全页面
  --slowmo <ms>                     慢动作执行，毫秒
  --timeout <ms>                    设置超时（默认 30000ms）

环境变量:
  PLAYWRIGHT_HEADLESS               设置默认 headless 模式
  PLAYWRIGHT_BROWSERS_PATH          设置浏览器路径
  PLAYWRIGHT_SLOWMO                 慢动作执行毫秒数

示例:
  playwright-cli navigate https://example.com
  playwright-cli screenshot myshot --fullpage
  playwright-cli click "button#submit"
  playwright-cli fill "input[name=email]" "test@example.com"
  playwright-cli screenshot mobile --mobile "iPhone 14"
  playwright-cli navigate https://example.com --browser firefox --headed
`);
}

// 解析选项
function parseOptions(args) {
  const options = {
    headless: process.env.PLAYWRIGHT_HEADLESS !== 'false',
    browser: 'chromium',
    viewport: { width: 1280, height: 720 },
    mobile: null,
    fullpage: false,
    slowmo: parseInt(process.env.PLAYWRIGHT_SLOWMO) || 0,
    timeout: 30000,
    state: 'visible'
  };

  for (let i = 0; i < args.length; i++) {
    const arg = args[i];
    if (arg === '--headed') {
      options.headless = false;
    } else if (arg === '--headless') {
      options.headless = true;
    } else if (arg === '--browser' && args[i + 1]) {
      options.browser = args[i + 1];
      i++;
    } else if (arg === '--viewport' && args[i + 1]) {
      const [w, h] = args[i + 1].split('x').map(Number);
      if (w && h) options.viewport = { width: w, height: h };
      i++;
    } else if (arg === '--mobile' && args[i + 1]) {
      options.mobile = args[i + 1];
      i++;
    } else if (arg === '--fullpage') {
      options.fullpage = true;
    } else if (arg === '--slowmo' && args[i + 1]) {
      options.slowmo = parseInt(args[i + 1]);
      i++;
    } else if (arg === '--timeout' && args[i + 1]) {
      options.timeout = parseInt(args[i + 1]);
      i++;
    } else if (arg === '--state' && args[i + 1]) {
      options.state = args[i + 1];
      i++;
    }
  }

  return options;
}

// 获取浏览器实例
async function getBrowser(options = {}) {
  if (!browser) {
    currentBrowserType = options.browser || 'chromium';

    const launchOptions = {
      headless: options.headless,
      slowMo: options.slowmo,
      args: [
        '--no-sandbox',
        '--disable-setuid-sandbox'
      ]
    };

    // 选择浏览器类型
    const browserType = currentBrowserType === 'firefox' ? firefox :
                        currentBrowserType === 'webkit' ? webkit :
                        chromium;

    browser = await browserType.launch(launchOptions);

    // 创建上下文
    if (options.mobile) {
      // 移动设备模拟
      const deviceDescriptor = devices[options.mobile];
      if (deviceDescriptor) {
        context = await browser.newContext({
          ...deviceDescriptor
        });
      } else {
        console.warn(`未知设备: ${options.mobile}，使用默认视口`);
        context = await browser.newContext({
          viewport: options.viewport
        });
      }
    } else {
      context = await browser.newContext({
        viewport: options.viewport
      });
    }

    page = await context.newPage();

    // 设置超时
    page.setDefaultTimeout(options.timeout);
    page.setDefaultNavigationTimeout(options.timeout);
  }
  return { browser, page, context };
}

// 导航命令
async function navigate(url, options) {
  if (!url) {
    console.error('错误: 需要提供 URL');
    process.exit(1);
  }

  const { page } = await getBrowser(options);

  try {
    await page.goto(url, { waitUntil: 'networkidle' });
    console.log(`✓ 已导航到: ${url}`);
    if (options.browser !== 'chromium') {
      console.log(`  浏览器: ${options.browser}`);
    }
  } catch (error) {
    console.error(`✗ 导航失败: ${error.message}`);
    process.exit(1);
  }
}

// 截图命令
async function screenshot(name = 'screenshot', options) {
  const { page } = await getBrowser(options);

  // 确保 screenshots 目录存在
  const screenshotDir = path.join(process.cwd(), 'screenshots');
  if (!fs.existsSync(screenshotDir)) {
    fs.mkdirSync(screenshotDir, { recursive: true });
  }

  const filepath = path.join(screenshotDir, `${name}.png`);

  try {
    // 等待页面稳定
    await page.waitForLoadState('networkidle');

    await page.screenshot({
      path: filepath,
      fullPage: options.fullpage
    });
    console.log(`✓ 截图已保存: ${filepath}`);
    if (options.fullpage) {
      console.log('  模式: 全页面');
    }
  } catch (error) {
    console.error(`✗ 截图失败: ${error.message}`);
    process.exit(1);
  }
}

// 点击命令
async function click(selector, options) {
  if (!selector) {
    console.error('错误: 需要提供选择器');
    process.exit(1);
  }

  const { page } = await getBrowser(options);

  try {
    await page.click(selector);
    console.log(`✓ 已点击: ${selector}`);
  } catch (error) {
    console.error(`✗ 点击失败: ${error.message}`);
    process.exit(1);
  }
}

// 填写表单命令
async function fill(selector, value, options) {
  if (!selector || value === undefined) {
    console.error('错误: 需要提供选择器和值');
    process.exit(1);
  }

  const { page } = await getBrowser(options);

  try {
    await page.fill(selector, value);
    console.log(`✓ 已填写: ${selector}`);
  } catch (error) {
    console.error(`✗ 填写失败: ${error.message}`);
    process.exit(1);
  }
}

// 选择下拉菜单
async function select(selector, value, options) {
  if (!selector || value === undefined) {
    console.error('错误: 需要提供选择器和值');
    process.exit(1);
  }

  const { page } = await getBrowser(options);

  try {
    await page.selectOption(selector, value);
    console.log(`✓ 已选择: ${selector} = ${value}`);
  } catch (error) {
    console.error(`✗ 选择失败: ${error.message}`);
    process.exit(1);
  }
}

// 悬停命令
async function hover(selector, options) {
  if (!selector) {
    console.error('错误: 需要提供选择器');
    process.exit(1);
  }

  const { page } = await getBrowser(options);

  try {
    await page.hover(selector);
    console.log(`✓ 已悬停: ${selector}`);
  } catch (error) {
    console.error(`✗ 悬停失败: ${error.message}`);
    process.exit(1);
  }
}

// 执行 JavaScript
async function evaluate(code, options) {
  if (!code) {
    console.error('错误: 需要提供 JavaScript 代码');
    process.exit(1);
  }

  const { page } = await getBrowser(options);

  try {
    // Playwright 使用不同的 evaluate 语法
    const result = await page.evaluate(code.includes('return') || code.includes('=>')
      ? new Function(code)
      : () => eval(code)
    );
    console.log('执行结果:', result);
  } catch (error) {
    console.error(`✗ 执行失败: ${error.message}`);
    process.exit(1);
  }
}

// 等待元素
async function wait(selector, options) {
  if (!selector) {
    console.error('错误: 需要提供选择器');
    process.exit(1);
  }

  const { page } = await getBrowser(options);

  try {
    await page.waitForSelector(selector, { state: options.state });
    console.log(`✓ 元素已${options.state === 'visible' ? '可见' : options.state === 'hidden' ? '隐藏' : '出现'}: ${selector}`);
  } catch (error) {
    console.error(`✗ 等待超时: ${error.message}`);
    process.exit(1);
  }
}

// 获取元素文本
async function getText(selector, options) {
  if (!selector) {
    console.error('错误: 需要提供选择器');
    process.exit(1);
  }

  const { page } = await getBrowser(options);

  try {
    const text = await page.textContent(selector);
    console.log(text);
  } catch (error) {
    console.error(`✗ 获取失败: ${error.message}`);
    process.exit(1);
  }
}

// 获取元素属性
async function getAttr(selector, attrName, options) {
  if (!selector || !attrName) {
    console.error('错误: 需要提供选择器和属性名');
    process.exit(1);
  }

  const { page } = await getBrowser(options);

  try {
    const value = await page.getAttribute(selector, attrName);
    console.log(value);
  } catch (error) {
    console.error(`✗ 获取失败: ${error.message}`);
    process.exit(1);
  }
}

// 键盘操作
async function press(key, options) {
  if (!key) {
    console.error('错误: 需要提供按键');
    process.exit(1);
  }

  const { page } = await getBrowser(options);

  try {
    await page.keyboard.press(key);
    console.log(`✓ 已按下: ${key}`);
  } catch (error) {
    console.error(`✗ 按键失败: ${error.message}`);
    process.exit(1);
  }
}

// 滚动到元素
async function scrollTo(selector, options) {
  if (!selector) {
    console.error('错误: 需要提供选择器');
    process.exit(1);
  }

  const { page } = await getBrowser(options);

  try {
    await page.evaluate((sel) => {
      const element = document.querySelector(sel);
      if (element) element.scrollIntoView({ behavior: 'smooth', block: 'center' });
    }, selector);
    console.log(`✓ 已滚动到: ${selector}`);
  } catch (error) {
    console.error(`✗ 滚动失败: ${error.message}`);
    process.exit(1);
  }
}

// 滚动到底部
async function scrollToBottom(options) {
  const { page } = await getBrowser(options);

  try {
    await page.evaluate(() => {
      window.scrollTo({ top: document.body.scrollHeight, behavior: 'smooth' });
    });
    console.log('✓ 已滚动到页面底部');
  } catch (error) {
    console.error(`✗ 滚动失败: ${error.message}`);
    process.exit(1);
  }
}

// 关闭浏览器
async function close() {
  if (browser) {
    await browser.close();
    browser = null;
    page = null;
    context = null;
    console.log('✓ 浏览器已关闭');
  }
}

// 主函数
async function main() {
  // 处理帮助
  if (!command || command === '--help' || command === '-h') {
    showHelp();
    process.exit(0);
  }

  // 解析选项
  const options = parseOptions(args);

  // 移除选项参数，保留命令参数
  const cleanArgs = args.filter((arg, i) => {
    if (arg.startsWith('--')) return false;
    if (args[i - 1] && args[i - 1].startsWith('--')) return false;
    return true;
  });

  switch (command) {
    case 'navigate':
    case 'nav':
    case 'goto':
      await navigate(cleanArgs[1], options);
      break;

    case 'screenshot':
    case 'shot':
    case 'ss':
      await screenshot(cleanArgs[1] || 'screenshot', options);
      break;

    case 'click':
      await click(cleanArgs[1], options);
      break;

    case 'fill':
    case 'type':
    case 'input':
      await fill(cleanArgs[1], cleanArgs.slice(2).join(' '), options);
      break;

    case 'select':
      await select(cleanArgs[1], cleanArgs[2], options);
      break;

    case 'hover':
      await hover(cleanArgs[1], options);
      break;

    case 'evaluate':
    case 'eval':
    case 'js':
      await evaluate(cleanArgs.slice(1).join(' '), options);
      break;

    case 'wait':
      await wait(cleanArgs[1], options);
      break;

    case 'text':
      await getText(cleanArgs[1], options);
      break;

    case 'attr':
      await getAttr(cleanArgs[1], cleanArgs[2], options);
      break;

    case 'press':
      await press(cleanArgs[1], options);
      break;

    case 'scroll-to':
      await scrollTo(cleanArgs[1], options);
      break;

    case 'scroll-to-bottom':
      await scrollToBottom(options);
      break;

    case 'close':
    case 'exit':
    case 'quit':
      await close();
      break;

    default:
      console.error(`未知命令: ${command}`);
      showHelp();
      process.exit(1);
  }
}

// 运行主函数
main().catch(error => {
  console.error('错误:', error.message);
  process.exit(1);
});

// 进程退出时关闭浏览器
process.on('exit', () => {
  if (browser) {
    browser.close().catch(() => {});
  }
});

process.on('SIGINT', async () => {
  await close();
  process.exit(0);
});
